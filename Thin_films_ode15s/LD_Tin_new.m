function [Reflectivity,Absorption,e1,e2]= LD_Tin(lambda,material,model,T1,T2,AB_new, BL)
...
hbar=6.582119e-16;  % in eV*sec


G0=(BL*AB_new.*(T1).^2+BL*(T2));
    epsilon_D = ones(size(lambda)) - ((f(1)*(omegap)^2) *...
        (omegalight^2 - i*G0*omegalight).^(-1));
end
...
db1=db*1e-6; % db in meters
e1=real(epsilon);
e2=imag(epsilon);



% surface_plasmon=lambda*1e9*real(sqrt((e1+1+e2*1i)/(e1+e2*1i)));
%  max(surface_plasmon(:))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% e1=linspace(-26,-1,300);
% e2=linspace(0,40,300);
% 
% [E1,E2]=meshgrid(e1,e2);
% 
% surface_plasmon_r=lambda*1e9*real(sqrt((E1+1+i*E2)./(E1+E2*i)));
% %surface_plasmon_i=lambda*1e9*imag(sqrt((E1+1+i*E2)./(E1+E2*i)));
% 
% figure;imagesc(e1,e2,surface_plasmon_r);
% [r,c]=find(surface_plasmon_r>890 &surface_plasmon_r<=900);
% hold on;plot(e1(c),e2(r),'k.');
% xlabel('real part of \epsilon')
% ylabel('imaginary part of  \epsilon')
% frf
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

f1=sqrt((e1+sqrt(e1.^2+e2.^2))/2);
f2=sqrt((-e1+sqrt(e1.^2+e2.^2))/2);


% f1=sqrt((e1+sqrt(e1.^2+e2.^2))/2);
% f2=sqrt((-e1+sqrt(e1.^2+e2.^2))/2);

Reflectivity=((f1-1).^2+f2.^2)./((f1+1).^2+f2.^2);

Absorption=2*omegalight*f2/3e14; % invesre microns
reflection_coeff=(-(f1-i*f2)+(1+i*0))./((f1-i*f2)+(1+i*0));
complex_refractive_index=f1-i*f2;

revised=(1/2)*1./(lambda/(4*pi)*(1./f2));

%%%%%%%%%%%propagation_constant1=2*pi*f1/lambda-i*revised;
%propagation_constant1=2*pi*complex_refractive_index/lambda;
%posotita=4*pi/lambda*f2.*(1./(1+(4*pi./lambda*f2*db1)));

posotita=2*pi*f2./(lambda);

propagation_constant1=2*pi*f1/lambda-i*posotita;


%propagation_constant2=2*pi*(f1+i*f2)/(lambda);

propagation_constant2=2*pi*f1/lambda+i*revised;

transm_coeff=2*(1+i*0)./((f1-i*f2)+(1+i*0));
